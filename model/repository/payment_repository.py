...
class PaymentRepository:
    ...
    def save(self, payment):
        self.connect()
        self.cursor.execute("insert into payments(transaction_type, payment_type, date_time, patient_id, total_amount, description) values (?, ?, ?, ?, ?, ?)",
    ...
    def update(self, payment):
        self.connect()
        self.cursor.execute("update payments set transaction_type=?, payment_type, date_time=?, patient_id=?, total_amount=?, description=? where id=?",
                            [payment.transaction_type, payment.payment_type, payment.date_time, payment.total_amount, payment.payment_id, payment.description])
        self.connection.commit()
        self.disconnect()
        return payment
